...
const getBooksById = async function (req, res) {
    try {
        ...
        return res.status(200).send({ status: true, message: "Book List with Reviews", data: { book} })
    }
    catch (error) {
...
const updateBook = async function (req, res) {
    ...
    try {
        ...
        if(ISBN != undefined){
            ISBN = ISBN.trim()
            if(!validator.isValidISBN(ISBN)){return  res.status(400).send({status:false, message:"enter a valid ISBN of 10 or 13 characters"})
            }
        }

        let checkISBN = await bookModel.findOne({ ISBN: requestBody.ISBN, isDeleted: false })
        if (checkISBN) {
